...
async function seedPostgres(db) {
  ...
  await db.transaction(async (tx) => {
    ...
    const postAdm = await tx.get(`SELECT id FROM job_posts WHERE LOWER(code) = LOWER(?)`, ['ADM']);
    ...
    if (!postOps || !postAdm || !titleEng || !titleAdm) {
    ...
    await tx.run(
      ...
      ['admin', 'admin@example.local', hashAdmin, 'System Admin', 'ADM01', postAdm.id, titleAdm.id]
    );
...
async function main() {
  ...
  console.log('Demo admin: job_post ADM (Administration (HQ)), job_title ADM (Administrator)');
